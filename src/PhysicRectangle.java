...
/**
 * Vad en rektangel med fysik måste innehålla.
 * 
 * @author BG5
 *
 */
public abstract class PhysicRectangle extends Rectangle {
	...
	/**
	 * @param didObjectIntersectWall
	 *            the didObjectIntersectWall to set
	 */
	...
	/**
	 * @param takeCareOfCollision
	 *            the takeCareOfCollision to set
	 */
	...
	/**
	 * @param floorZeroVelocity
	 *            the floorZeroVelocity to set
	 */
	...
	/**
	 * @param didObjectIntersectFloor
	 *            the didObjectIntersectFloor to set
	 */
	...
	// Tittar om två objekt kolliderar med varandra.
		...
		if (pr.intersects(r)) {
			if (pr.getCollideNumbers().contains(r.getColliderNumber())) {
				if (r.getColliderNumber() == Node.Collision.SOLIDOBSTACLE
						|| r.getColliderNumber() == Node.Collision.BOINKOBSTACLE) {
					if (r.getColliderNumber() == Collision.BOINKOBSTACLE
					...
					if ((pr.isInAir() && y1 <= r.getPosition().y || y2 >= r
							.getPosition().y + r.getHeight())
							|| this.didObjectIntersectWall
							&& y1 > r.getPosition().getY()) {
						if (pr.getVelocity().getY() >= 0) {
							...
							if (r.getColliderNumber() == Node.Collision.BOINKOBSTACLE) {
								vy = (float) ((pr.getMass() - r.getMass())
										/ (pr.getMass() + r.getMass()) * pr
										.getVelocity().getY());

								if (r instanceof Obstacle)
									vy *= ((Obstacle) r).getBoinkFactor(); // dont
																			// want
																			// all
																			// the
																			// of
							...
							pr.setPosition(new Point(pr.getPosition().x, r
									.getPosition().y - pr.getHeight()));
						} else {
						...
						if (pr.getVelocity().getX() > 0) {
							...
							if (r.getColliderNumber() == Node.Collision.BOINKOBSTACLE) {
								...
								vx = (float) ((pr.getMass() - r.getMass())
										/ (pr.getMass() + r.getMass()) * pr
										.getVelocity().getX());

								if (r instanceof Obstacle)
									vx *= ((Obstacle) r).getBoinkFactor(); // dont
																			// want
																			// all
																			// the
																			// of
							...
							pr.setVelocity(new Velocity(vx, pr.getVelocity()
									.getY()));
							pr.setPosition(new Point(r.getPosition().x
									- pr.getWidth() - 1, pr.getPosition().y));
					...
					if ((x1 >= r.getPosition().x || x2 <= r.getPosition().x
							+ r.getWidth())) {
						if (this.getVelocity().getX() > 0) {
							this.setPosition(new Point(r.getPosition().x
									- this.getWidth(), r.getPosition().y));
						} else if (this.getVelocity().getX() < 0) {
							this.setPosition(new Point(r.getPosition().x
									+ r.getWidth(), r.getPosition().y));
						}
					}

					Map.physics.calculateElasticCollision(this,
							(PhysicRectangle) r);
				}
				return true;
