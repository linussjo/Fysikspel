...
/**
 * Huvudklass för att rita ut bilden. Håller även koll på vad som händer vid
 * olika knapptryckningar.
 * 
 * @author BG5
 *
 */
public class Component extends JComponent {
	...
	public Component(Map map) {
		...
		try {
			background = ImageIO.read(new File(FileSystems.getDefault()
					.getPath("data", "background.png").toUri()));
		} catch (IOException e) {
	...
	protected void paintComponent(Graphics g) {
		...
		for (Item i : player.getItemContainer()) {
			if (player.getActiveItem() == i) {
				...
				g2.fillRect(x - 15, this.HEIGHT - 50 - i.getHeight(),
						i.getWidth() + 30, i.getHeight() + 30);
			}
			g2.drawImage(i.getImage(), x, this.HEIGHT - 35 - i.getHeight(),
	...
	// Vad som händer vid olika knapptryckningar.
	private void setInput() {
		...
		getActionMap().put("SPACE", new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (map.getPlayer().getActiveItem() instanceof Bow
						&& !map.getPlayer().shotArrow()) {
